...
The app calls PokéAPI for four kinds of read-only reference data: the
...
app needs to observe — a species's base EV yield or its evolution chain
...
lookup — `pokemon-species` and the evolution-chain endpoint), and every
...
1. **Every** network call PokeApiClient makes goes through a single
   `_cached(key, fetcher)` helper — there is no `fetch()` outside it. A
...
- Storage growth is unbounded in theory, but bounded in practice: the
  entire PokéAPI species catalog is ~1300 small JSON records, which is
  negligible against typical `localStorage` quotas (5–10MB).
- There is deliberately no cache invalidation/expiry. If PokéAPI ever
