...
- **UI entry point:** a "Where to train" button directly under `<ev-summary>` on the Pokémon detail page (`components/caught-pokemon-detail.js`), opening a dialog that hosts `<ev-training-guide>` (`components/ev-training-guide.js`) — one `<item-button-grid>` section per stat, reusing the same sprite+label+boost row every other item grid in this file already uses. It's a standalone entry point, not a "More" menu item, so docs/adr/0014's gen-gated-ordering rule doesn't apply to it; if it ever moves into that menu, it'd land after Competitive (both are effectively Gen III+ in practice) and before Release.
