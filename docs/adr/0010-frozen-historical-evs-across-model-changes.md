# 10. Historical events default to frozen â the Gen I/II Stat Experience backfill is the deliberate, narrow exception
...
PokÃ©mon's own base stat (0-65,535 per stat, no combined total cap)
...
PokÃ©API data can't reconstruct.
...
"the data isn't valid under the new rules anymore" â shouldn't this
route through it? It doesn't, and the two situations are different:

- ADR-0009's migrations exist for a *shape* break â a field renamed,
  ...
  replay correctly with these fields absent â the
...
requires re-fetching from PokÃ©API â a network dependency that doesn't
...
overwhelmingly common case, still sitting in `localStorage` right now â
...
1. **A historical event's frozen value is never rewritten by an
   ...
   deliberate, narrow, one-time exception to that default** â justified
...
2. **`Store#_backfillGen1StatExp()`** runs once per install, guarded by
   `state.statExpBackfillApplied` (its own flag â deliberately *not*
   ...
   - Recomputes every `vitamin` event unconditionally (its amount never
     depended on the opponent â nothing can be missing).
   - Recomputes every `battle` event via `PokeApiClient#peekCached`
     (a new synchronous, cache-only lookup â memory tier then
     `localStorage`, **never** a network call) for the opponent's base
     stats, applying the same Gen I merged-Special and PokÃ©rus-doubling
     logic `logBattle` itself uses.
   - Leaves a `battle` event's `applied` **untouched** if its opponent
     isn't cached (cleared site data, private browsing) â best-effort,
...
3. **`Store`'s constructor takes an optional `peekCachedMon` dependency**
   ...
   so `Store` itself stays decoupled from PokÃ©API's cache-key format â
...
4. **No `MIGRATIONS` entry, no `SCHEMA_VERSION` bump, no release-notes
   "Breaking changes" entry** â ADR-0009 reserves that machinery for
...
- Anyone who already had a Gen I/II party with battle history before
  ...
  API cache â which, given ADR-0001's no-expiry policy, is expected to
...
- A battle event whose opponent fell out of cache stays at its old,
  wrong scale forever, silently â no UI currently surfaces "N events
  ...
  "recompute this PokÃ©mon" action) would need to solve that
  reporting gap for real if it turns out to matter.
- This does *not* generalize into "future rules corrections get
  automatic backfills by default" â point 1's default (frozen forever)
  ...
  before assuming either a migration *or* a backfill is required â the
  default is neither.

## Addendum — the backfill moved into `Store#init()` (ADR 0025)

[ADR 0025](0025-persistence-layer-and-indexeddb.md) P4b moved the Gen
I/II Stat-Exp backfill **out of `Store`'s synchronous constructor** and
into `async Store#init()`, after the PokéAPI cache is warmed into memory
(`hydrateCache`) and after the roster is loaded from the IndexedDB rows.
It now runs against a warm `peekCached` and the adopted roster, and
`_runStatExpBackfill` re-projects and persists what `_backfillGen1StatExp`
corrects (the latter returns `touched` instead of saving itself).

Everything this ADR decides is unchanged: still one-time (guarded by
`statExpBackfillApplied`), still fully local (`peekCachedMon` —
`PokeApiClient#peekCached` — never a network call), still best-effort
(a battle whose opponent isn't cached is left alone). Tests that load a
Gen I/II legacy save now `await new Store(...).init()`.
