...
Â§7 gave the persisted state a `schema` number and one hardcoded
`_migrateV1`, run automatically and silently inside `Store#_load()` the
instant a mismatch is seen â accepted at the time as "pre-production,
...
"Breaking changes" section for exactly this case â the human-facing
...
- **An explicit `breaking: true/false` field** alongside each migration
  step, independent of the version number â rejected as a second value
  that could drift from the number itself.
- **A `major.minor` schema version**, deriving "is this breaking" from
  the major component â rejected once it became clear `minor` was
...
- **A user-triggered "Run update now" gate** that deferred the app's own
  code update (not just the data write) until Settings was visited â
  ...
  a native app's "an update is available" ritual â
  ...
  state â complexity this design does without.
- **Deriving the schema version from the app's own SemVer tag directly**
  (fetched over the network) instead of a hardcoded local constant â
  ...
  the tab's *actual* running code is older â precisely the staleness
  case ADR-0004 Â§4 exists to catch. Reusing that fetched value for this
  check would silently defeat itself in exactly that scenario.
- **A dedicated file per migration** (e.g. `v2.0.0.js`) â rejected: this
...
- **A Rails/Django-style set of independently-numbered (or timestamped)
  migrations with an "applied" tracker** â rejected. That model exists
...
2. **`MIGRATIONS` in `lib/store.js`** is an ordered array of
   ...
   Every entry is a breaking change by definition â anything
...
3. **Migrations run automatically and fully at load, no gate.**
   ...
   sequence â an install can be arbitrarily far behind and still catch
   ...
   `MIGRATIONS` entry â corrupt data, or a version newer than this build
...
4. **A local backup is the safety net, not a gate.** The instant
   ...
   `localStorage` â before anything else has a chance to overwrite the
...
5. **Two tests guard against the actual mistake this whole ADR is
   about** â shipping a shape change without a migration for it:
   - `test/store.test.js`'s *"SCHEMA_VERSION and the MIGRATIONS chain
     ...
     ends exactly at `SCHEMA_VERSION` â catches "bumped the version,
   ...
   - `test/store.test.js`'s *"a real save frozen at schema 1 still loads
     ...
     (`test/fixtures/state-schema-1.json`, covering every event kind â
     catch, battle, vitamin, feather, berry, pokerus, exp-share, level,
     evolve â generated once via `test/fixtures/generate-state-schema-1.mjs`
     and never regenerated to "fix" a failure) and asserts specific
     values still come out right. If a future change breaks
     compatibility with it, this test fails â not a real user's save.
...
4. Copy `test/fixtures/generate-state-schema-1.mjs` forward (e.g.
   ...
   alongside the existing one (the old fixture and its test stay â
...
- The residual risk this ADR accepts: a migration bug could still
  ...
  auto-migration, with only the local, non-UI backup as a safety net â
  ...
  is ever actually needed, but isn't built yet â there's no real
  migration to test it against.
- `lib/schema-version.js` is now a purely internal, offline concern of
  `lib/store.js` (and the test suite) â nothing in the update-check
...
## Addendum — a second migration chain, for the object stores (ADR 0025)

[ADR 0025](0025-persistence-layer-and-indexeddb.md) moves the roster
into IndexedDB and adds a **parallel** version/migration mechanism of
the same shape as this one, for the object stores:
`lib/db/schema.js` holds `DB_VERSION` + an ordered `MIGRATIONS` array of
frozen schema deltas, applied in `onupgradeneeded`, guarded by
`test/db-schema.test.js` (the analogue of this ADR's
`SCHEMA_VERSION`/`MIGRATIONS`-agree + frozen-fixture tests).

The blob's own `schema`/`MIGRATIONS` chain and everything in this ADR is
**unchanged** — the one-time blob → rows import (ADR 0025 §6) runs the
existing `_load` → `_migrateV1`/`MIGRATIONS` → `_normalizeEntries`
pipeline first, then writes the normalized result to rows. The
pre-migration backup gains a sibling: `effortdex:state.pre-idb-backup`
(ADR 0025 P4d, pending).
