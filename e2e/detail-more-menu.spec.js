...
// IVs / Competitive) instead of one big dialog directly — split so a
// quick competitive or IV check doesn't scroll past training controls,
// and vice versa.
...
test.describe('Detail page "More" menu', () => {
  ...
  test('picking "IVs" opens the IVs dialog specifically, not the training one', async ({ page }) => {
    await page.goto('/');
    await createParty(page, { name: 'Emerald Nuzlocke', baseGame: 'Emerald' });
    await catchPokemon(page, 'Bulbasaur');
    const card = await openDetail(page, 'Bulbasaur');
