...
// Release) rather than a dialog directly. Level, the held-item picker
...
// menu itself is just IVs, Competitive, and the release confirmation.
...
test.describe('Detail page "More" menu', () => {
  ...
  test('clicking More opens a menu with all three options, not a dialog directly', async ({ page }) => {
    ...
    await expect(card.getByRole('menuitem', { name: 'Release' })).toBeVisible();
  ...
  test('picking "Release" asks for confirmation via the native dialog, no popup of its own', async ({ page }) => {
    await page.goto('/');
    await createParty(page, { name: 'Emerald Nuzlocke', baseGame: 'Emerald' });
    await catchPokemon(page, 'Bulbasaur');
    const card = await openDetail(page, 'Bulbasaur');
    ...
    await expect(card).toBeVisible(); // still here — dismissing the confirm cancels the release
  });
