 ...
 * and show it.
 */
export class IvDialog extends HTMLElement {
  constructor() {
    super();
    ...
    this.$dialog = /** @type {HTMLDialogElement} */ (shadow.querySelector('.iv-dialog'));
    this.$close = shadow.querySelector('.iv-dialog-close');
    ...
    // Same Enter-to-Save convention as the rest of caught-pokemon-
    // detail's dialogs — a native <dialog> with no <form> otherwise just
    // swallows Enter.
    ...
    this.$helpBtn?.addEventListener('click', () => {
      const anchor = this.$helpBtn.closest('.ds-dialog-header');
      if (!anchor) return;
