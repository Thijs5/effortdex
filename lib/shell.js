...
/* ------------------------------------------------------------------ */
/* "Storage is full" banner                                            */
/* ------------------------------------------------------------------ */

// Store keeps working from memory when a save fails, but the user needs
// to know their edits won't survive a reload until they free space (the
// Storage page's "Clear cache" now drops the localStorage PokéAPI cache
// too). Store#_save flips `saveHealthy` and fires save-error / save-ok
// on each transition.
const storageWarning = requireElementById('storage-warning');
const showStorageWarning = (/** @type {boolean} */ show) => {
  storageWarning.hidden = !show;
};
store.addEventListener('save-error', () => showStorageWarning(true));
store.addEventListener('save-ok', () => showStorageWarning(false));
store.addEventListener('save-gap', () => showStorageWarning(true)); // a recent change didn't persist (docs/adr/0025 P4d)
showStorageWarning(!store.saveHealthy);
