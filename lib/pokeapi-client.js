...
export class PokeApiClient {
  ...
  constructor({ cacheBackend } = {}) {
    ...
    // Best-effort: pull already-cached `getPokemon` results into the
    // in-memory tier so `peekCached` (used by Store's one-time Gen I/II
    // backfill, docs/adr/0010) can still see them now that the disk tier
    // is async. Racy with an immediate peek, but that backfill is a
    // no-op for every save written since ADR 0010.
    void this._cache.warm([MON_KEY_PREFIX]);
  }
