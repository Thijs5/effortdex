  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  @media (max-width: 640px) {
    .ds-dialog {
      ...
      width: 100vw;
      max-width: 100vw;
      /* No height declaration here on purpose: a dialog's UA default is
         height:fit-content, so it sizes to its content and max-height
         just caps a tall one at the viewport (then overflow-y:auto above
         scrolls it). Forcing height:100dvh instead made every short
         dialog a full-screen sheet with a big dead band of blank panel
         below the content and its action row stranded mid-screen — and,
         because a modal dialog's UA inset:0 sets both top and bottom, an
         abs-pos box with an auto height would stretch edge to edge
         anyway; fit-content is what sidesteps that (the BaseDialog
         mobile override leans on the same trick). margin:0 still pins it
         to the top so an opening keyboard can't shove a field off. */
      max-height: 100dvh;
      border-radius: 0;
    }
  }
