...
const css = `
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .ds-dialog {
    ...
    /* No top or bottom padding: the sticky .ds-dialog-header/-footer own
       that space themselves (a dialog with no footer, e.g. Competitive,
       adds its own bottom padding to its content instead) — giving both
       here too would double up with each one's own padding. (A negative
       top/bottom margin on the header/footer instead would break their
       stickiness — sticky constraints use the margin box, so it would
       pin past the scrollport edge and let content peek out beyond it.) */
  ...
  .ds-dialog-footer {
    ...
    /* Horizontal + bottom bleed only, canceling .ds-dialog's own (zero)
       padding there — no top margin: the dialog's grid already places
       var(--space-4) of gap above every row including this one, so an
       additional margin-top here would just double that spacing. */
    margin: 0 calc(-1 * var(--space-5)) calc(-1 * var(--space-5));
    /* Bottom padding matches .ds-dialog's own left/right (--space-5) —
       same reasoning as the header's own top padding above. */
  ...
  .ds-dialog-header {
    ...
    gap: var(--space-3);
    /* Horizontal bleed only, canceling .ds-dialog's own padding there —
       no bottom margin: the dialog's grid already places var(--space-4)
       of gap below every row including this one, so an additional
       margin-bottom here would just double that spacing. */
    margin: 0 calc(-1 * var(--space-5)) 0;
    /* Top padding matches .ds-dialog's own left/right (--space-5) —
       this is the dialog's real top inset, since .ds-dialog itself
       carries none (see its own comment: the sticky header owns it). */
