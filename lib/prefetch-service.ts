...
// per-game trigger (components/pages/settings/cache.js) added alongside
// it, and its "evolution chain prefetch" addendum for the evolution-data
// warming below. sw.js caches a sprite the moment it's ever requested,
// and PokeApiClient caches an evolution chain the same way (ADR 0001) —
// but a species nobody has looked up yet, or whose evolution UI nobody
// has opened yet, has never been requested, so both still 404/fail
// offline even though it's squarely within a generation the user cares
// about.
//
// All entry points — the automatic scan and the manual per-game/
// per-generation triggers — feed into one shared, concurrency-limited
// queue rather than each running its own fetch loop. That's deliberate:
// without it, an automatic scan already in flight plus someone mashing
// two or three "cache this game" buttons would mean several independent
// loops each opening their own concurrent connections to PokéAPI at
// once. A single queue means the *total* in-flight request count is
// always bounded by `concurrency`, no matter how many callers ask for
// work at once — new work just extends the same queue instead of
// competing for bandwidth with it. Evolution-chain warming shares this
 ...
 * URL outright (`url`), or — for evolution-chain warming — just runs an
...
export class PrefetchService extends EventTarget {
   ...
   * `itemSpriteUrls` defaults to the real fixed set (lib/constants.ts's
   ...
   * into.
   * Also enqueues (fire-and-forget, not part of `onProgress`'s count —
  ...
  async prefetchGame(gameName: string, onProgress?: (progress: { done: number; total: number }) => void): Promise<void> {
    ...
    try {
      ...
      this._enqueueEvolutionChains(species);
  ...
  /**
   * Enqueues evolution-chain warming (`PokeApiClient#getEvolutionChain`)
   * for `species`, so a species' evolution family is already sitting in
   * PokeApiClient's own cache (ADR 0001) by the time anything actually
   * opens its evolution UI — closing the same kind of offline gap ADR
   * 0011 closed for sprites, for evolution data instead.
   * Fire-and-forget, like the item-icon warming above: not awaited by
   * any caller, and deliberately left out of `onProgress`'s `{done,
   * total}` count — this should stay invisible background work
   * regardless of which entry point (automatic scan or a manual
   * button) triggered it, the same "no visible progress" call ADR 0011
   * made for the sprite scan itself.
   * Always enqueued under the shared `'evo'` source tag, never a
   * per-game one like sprite tasks sometimes are — an evolution chain
   * doesn't vary by title the way a sprite can, so there is exactly one
   * correct target per species regardless of which game asked for it,
   * and every caller should merge into the same dedup entry.
  ...
  private async _runTask(task: QueueTask): Promise<void> {
    ...
    try {
      ...
      if (warm) {
        ...
        // fetch — `warm()` itself (PokeApiClient#getEvolutionChain)
        // does the actual network call and populates PokeApiClient's
        // own cache as a side effect (ADR 0001), same as `getPokemon`
        // does for sprite tasks below.
        await run(() => warm());
      } else {
        // An item-icon task already knows its URL outright — no species
        // lookup needed, unlike every other sprite task this queue
        // handles.
