...
export class Store extends EventTarget {
  ...
  _normalizeEntries() {
    ...
    for (const party of this.state.parties) {
      ...
      for (const entry of party.pokemon) {
        ...
        if (typeof entry.baseStats !== 'object' || entry.baseStats === null) entry.baseStats = null;
      }
    }
  }
  ...
  catchPokemon(mon, level = DEFAULT_LEVEL, natureId = null) {
    ...
    const clampedLevel = Number.isNaN(parsedLevel)
      ? DEFAULT_LEVEL
      : Math.min(MAX_LEVEL, Math.max(MIN_LEVEL, parsedLevel));
    const entry = {
      ...
      machoBrace: false,
      pokerus: false,
      // Seeds the history log with the origin event, so it always shows
      // where the Pokémon's stats started rather than opening blank.
      history: [{ id: crypto.randomUUID(), kind: 'catch', level: clampedLevel, timestamp: Date.now() }],
      evolutions: [],
    };
  ...
  /** Logs a history entry whenever Pokérus is actually toggled on or off, so the roster's EV log shows when the ×2 boost started (or stopped). */
  setPokerus(uid, val) {
    const entry = this._find(uid);
    if (!entry) return;
    const next = !!val;
  ...
  /** Logs a history entry for any actual level change, from either the level-up button or a manual edit. */
  setLevel(uid, level) {
    const entry = this._find(uid);
    if (!entry) return;
  ...
  _battleYield(entry, opponent) {
    ...
    if (entry.pokerus && this.pokerusAvailable()) {
  ...
  /**
   * Deletes one history record (battle, vitamin or level-up) from the
   * caught Pokémon `uid` — for a mislogged entry — and reverts whatever
   * it applied, so the roster stays consistent with what's actually
   * displayed in the history list. A vitamin record's `applied` is a
   * number; a battle record's is a per-stat object, same shape as `evs`;
   * a level record reverts `level` back to `fromLevel`.
   */
  deleteHistoryEntry(uid, historyId) {
    const entry = this._find(uid);
    if (!entry) return;
    const hist = entry.history.find((h) => h.id === historyId);
    if (!hist) return;
    if (hist.kind === 'vitamin') {
      entry.evs[hist.stat] -= hist.applied;
    } else if (hist.kind === 'level') {
      entry.level = hist.fromLevel;
    } else if (hist.kind === 'pokerus') {
      entry.pokerus = !hist.active;
    } else if (hist.kind === 'battle') {
      for (const { key } of STATS) entry.evs[key] -= hist.applied[key] || 0;
    }
    // 'catch' has nothing to revert — it's the origin record, not
    // something that applied EVs or a level change on its own.
    entry.history = entry.history.filter((h) => h.id !== historyId);
    this._save();
  }
}
