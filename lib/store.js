...
export class Store extends EventTarget {
   ...
   *   Best-effort: writes the current roster state into the IndexedDB
   *   `parties`/`rosterEntries`/`events` stores (docs/adr/0025 P4a). The
   *   localStorage blob is still authoritative; this keeps a verified
   *   shadow copy. Wired in `services.js`; absent (skipped) by default
   *   and when IndexedDB is unavailable.
   */
  constructor(deps = {}) {
    super();
  ...
  async init() {
    ...
    await this._hydrateCache().catch(() => {}); // best-effort — a cold cache just means a few refetches
    // P4a: one-time blob -> IndexedDB rows import. Best-effort while the
    // blob is still authoritative — a failure is logged, not fatal.
    if (this._mirrorRoster) {
      try {
        await this._mirrorRoster(this.state, { firstRunOnly: true });
      } catch (err) {
        console.error('effortdex: roster import to IndexedDB failed (blob still authoritative)', err);
      }
    }
    return this;
  }
  ...
  _save() {
    ...
    if (this._writeState(JSON.stringify(persisted))) return;
  ...
  _writeState(json) {
    ...
    // P4a: keep the IndexedDB shadow copy fresh after every persisted
    // mutation. Fire-and-forget — the blob write above is what matters
    // until P4b flips the read. (P4c replaces this whole-roster rewrite
    // with targeted per-mutation row writes.)
    if (this._mirrorRoster) void Promise.resolve(this._mirrorRoster(this.state)).catch(() => {});
    this.dispatchEvent(new CustomEvent('change'));
    return true;
  }
