...
import { STATS, POWER_ITEMS, VITAMINS, STAT_CAP, TOTAL_CAP, POWER_ITEM_BONUS, VITAMIN_BONUS, VITAMIN_STAT_CUTOFF, VITAMIN_CUTOFF_MIN_GEN, VITAMIN_CUTOFF_MAX_GEN, MIN_LEVEL, MAX_LEVEL, DEFAULT_LEVEL } from './constants.js';
...
export class Store extends EventTarget {
   ...
   * base yield adjusted for the entry's power item and Pokérus, then
   * clamped to the per-stat (252) and total (510) caps given the entry's
   * *current* EVs. Read-only: never mutates `entry`.
   */
  _battleYield(entry, opponent) {
    ...
    if (itemDef) afterItem[itemDef.stat] = (afterItem[itemDef.stat] || 0) + POWER_ITEM_BONUS;
   ...
   * `uid`, honoring its power item (+8 flat) and Pokérus (×2), then
   * clamping to the per-stat (252) and total (510) caps.
   */
