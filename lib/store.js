 ...
 * @property {string[]|null} availableSpecies
 */
...
// `availableSpecies` is the other: it doesn't touch EV math at all, only
// which species the battle-log/add-to-roster pickers offer (see
// lib/species-availability.js) — for a randomizer/mod whose actual dex
// doesn't match any real game's generation.
...
function defaultOverrides() {
  return {
    ...
    availableSpecies: null, // null | non-empty species-name[] — restricts pickers to exactly this list (mods/randomizers), see lib/species-availability.js
  };
}
...
export class Store extends EventTarget {
  ...
  setExpShare(uid, val, batchId) {
    ...
    if (next && (entry.powerItem !== null || entry.machoBrace)) {
      ...
      entry.machoBrace = false;
      this._append(entry, makeEvent('held-item', { powerItem: null, machoBrace: false }, batchId));
    }
