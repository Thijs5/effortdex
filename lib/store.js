 ...
 * @param {boolean} [trackIvs]
 * @returns {Party}
 */
function makeParty(name, description, baseGame, overrides, existingSlugs, trackIvs = false) {
  return {
    id: uuidv7(),
    name,
    description,
...
export class Store extends EventTarget {
  ...
  _normalizeEntries() {
    ...
    for (const party of /** @type {any[]} */ (this.state.parties)) {
      ...
      if (typeof party.trackIvs !== 'boolean') party.trackIvs = false;
    ...
    for (const party of /** @type {any[]} */ (this.state.parties)) {
      for (const entry of party.pokemon) {
        ...
        if (typeof entry.trackIvs !== 'boolean') entry.trackIvs = false;
   ...
   * @returns {Party}
   */
  createParty(name, description = '', baseGame = '', overrides = {}, trackIvs = false) {
  ...
  /**
   * Updates a party's name/description/base game/rule overrides/IV
   * tracking. The slug (and its URL) never changes. `overrides` is
   * merged over the party's existing overrides (per-key), not replaced
   * wholesale.
   */
  /**
   * @param {string} id
   * @param {{ name?: string, description?: string, baseGame?: string, overrides?: Partial<PartyOverrides>, trackIvs?: boolean }} changes
   */
  updateParty(id, { name, description, baseGame, overrides, trackIvs }) {
    ...
    if (trackIvs !== undefined) party.trackIvs = trackIvs;
    this._save();
  }
  ...
  setPartyTrackIvs(id, value) {
    const party = this.state.parties.find((p) => p.id === id);
    if (!party) return;
    party.trackIvs = value;
    this._save();
  }

  /** @param {string} uid @param {boolean} value */
  setEntryTrackIvs(uid, value) {
    const entry = this._find(uid);
    if (!entry) return;
    entry.trackIvs = value;
    this._save();
  }
   ...
   * powerItem, machoBrace, trackIvs, ivs) are left untouched — they
   * aren't journaled, so there's nothing principled to merge them by.
   */
    ...
    for (const party of importedParties) {
      ...
      if (!localParty) {
        ...
        localParty = {
          ...
          trackIvs: /** @type {any} */ (party).trackIvs ?? false,
