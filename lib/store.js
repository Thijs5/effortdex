...
export class Store extends EventTarget {
  ...
  /**
   * This Pokémon's real current value for `statKey` — base + IV + EV +
   * level + nature, the exact formula `possibleIvsForStat` brute-forces
   * against — or null if this stat's IV isn't known yet (nothing
   * entered or narrowed down via a logged reading) or this generation
   * uses Stat Experience instead of IVs/EVs (Gen I/II's own stat
   * rounding is a distinct, unsourced formula not implemented here — see
   * `possibleIvsForStat`'s own comment).
