...
export class Store extends EventTarget {
   ...
   * @param {() => unknown} [deps.relieveStoragePressure]
   *   Called once by `_save()` when a `localStorage` write throws (quota
   *   exhausted — the common failure on an installed iOS PWA, where the
   *   PokeApiClient cache can fill the origin's whole bucket), to free
   *   space before one retry. Wired to `PokeApiClient#evictLocalCache`
   *   in `services.js`; defaults to a no-op so Store stays standalone.
   */
  constructor(deps = {}) {
    super();
    this._peekCachedMon = deps.peekCachedMon ?? (() => null);
    this._relieveStoragePressure = deps.relieveStoragePressure ?? (() => {});
    /**
  ...
  _save() {
    ...
    if (this._writeState(json)) return;

    // The write threw — almost always the storage quota. On an installed
    // iOS PWA the PokeApiClient localStorage cache (the per-generation
    // species lists especially) can exhaust the origin's bucket on its
    // own, leaving no room for the roster that actually matters. That
    // cache is disposable — every entry is refetched on demand — so drop
    // it and try the one write we can't afford to lose again.
    this._relieveStoragePressure();
    if (this._writeState(json)) return;

    // Still no room. Keep the in-memory state fully usable (the caller's
    // dialog still closes, the UI still re-renders below) but stop
    // pretending it's saved: flip `saveHealthy` and fire `save-error`
    // once, so shell.js can show a persistent "not saving" banner
    // instead of the user losing work silently on the next reload.
    if (this.saveHealthy) {
