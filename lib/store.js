...
/** @typedef {{ id: string, kind: 'stat-reading', timestamp: number, statKey: StatKey, level: number, evs: EvMap, observedStat: number }} StatReadingEvent */

/** @typedef {CatchEvent|BattleEvent|VitaminEvent|FeatherEvent|BerryEvent|ImportedEvent|PokerusEvent|ExpShareEvent|LevelEvent|EvolveEvent|StatReadingEvent} RosterEvent */
...
export class Store extends EventTarget {
   ...
   * @param {string} uid @param {StatKey} statKey @param {number} observedStat
   */
  logStatReading(uid, statKey, observedStat) {
    const entry = this._find(uid);
    if (!entry || this.usesStatExpSystem()) return;
    this._append(entry, makeEvent('stat-reading', { statKey, level: entry.level, evs: { ...entry.evs }, observedStat }));
  }
  ...
  /** Setting a power item, the Macho Brace, and Exp. Share are all mutually exclusive — one held item slot, and Exp. Share is itself a held item.
  ...
  setMachoBrace(uid, val) {
    ...
    this._save();
    if (entry.machoBrace) this.setExpShare(uid, false);
  }
