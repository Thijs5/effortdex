...
export class Store extends EventTarget {
   ...
   *   dual-write backup).
   * @param {(ops: import('./db/roster-ops.js').RosterOp[], meta: { rev: number, activePartyId: string|null }) => Promise<unknown>} [deps.rosterOps]
  ...
  async init() {
    ...
    if (this._mirrorRoster) {
      try {
        ...
        if (this._loadRoster) {
          ...
          const blobRev = this.state.rev ?? 0;
          const rowsRev = rows.rev ?? 0;
          const rowsHaveData = rows.parties.length > 0 || rowsRev > 0;
          if (rowsHaveData && rowsRev >= blobRev) {
            this._adoptRosterFromRows(rows, rowsRev);
          } else if (blobRev > rowsRev) {
            await this._mirrorRoster(this.state); // blob is newer — heal the shadow
          }
        }
      } catch (err) {
    ...
    //    and the adopted roster. Persists (blob + rows) if it corrected
    //    anything.
  ...
  /**
   * @param {import('./db/roster-ops.js').RosterOp[]} [idbOps] - when
   *   given, the IndexedDB side persists just these targeted ops
   *   (docs/adr/0025 P4c) instead of rewriting all three roster stores.
   *   The localStorage blob is always written in full. Omit for
   *   structural mutations (create/delete/reorder party or entry).
   */
  _save(idbOps) {
    this.state.rev = (this.state.rev ?? 0) + 1;
    const persisted = {
      schema: SCHEMA_VERSION,
      // docs/adr/0025 P4b: written into the blob and mirrored to
      // `meta.rosterRev`, so `init()` can tell which copy is current.
      rev: this.state.rev,
    ...
    if (this._writeState(JSON.stringify(persisted), idbOps)) return;

    // The write threw — localStorage is full or unavailable. Since
    // docs/adr/0025 P2 the PokéAPI cache lives in IndexedDB, not here,
    // so there is nothing left to evict to make room. Keep the in-memory
    // state fully usable (the caller's dialog still closes, the UI still
    // re-renders below) but stop pretending it saved: flip `saveHealthy`
    // and fire `save-error` once, so shell.js shows a persistent "not
    // saving" banner instead of the user losing work silently on reload.
    if (this.saveHealthy) {
  ...
  /** Attempts the actual state write. Returns true if it landed; on
   * success also clears any prior `save-error` condition (firing
   * `save-ok`) and dispatches `change`, so callers get identical
   * behavior to a plain successful save.
   * @param {string} json @returns {boolean} */
  /** @param {string} json @param {import('./db/roster-ops.js').RosterOp[]} [idbOps] */
  _writeState(json, idbOps) {
    let blobOk = true;
    try {
    ...
    // Mirror the roster into the IndexedDB rows — the read path since
    // docs/adr/0025 P4b — regardless of whether the blob write landed: a
    // full localStorage must not also block the roster's real home.
    // Fire-and-forget with the bumped `rev`; a reload that beats this
    // reconciles via `rev` against the blob. Targeted ops (P4c) when the
    // caller gave them, else a whole-roster rewrite.
    if (idbOps && this._rosterOps) {
      void Promise.resolve(
        this._rosterOps(idbOps, { rev: this.state.rev ?? 0, activePartyId: this.state.activePartyId })
      ).catch(() => {});
    } else if (this._mirrorRoster) {
