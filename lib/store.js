...
/** One display record per logged event — the event itself for every kind
 * except 'level' and 'evolve', which EVENT_HANDLERS below enriches with
 * fold context (e.g. `fromLevel`) that isn't on the stored event.
 * @typedef {CatchEvent|BattleEvent|VitaminEvent|FeatherEvent|BerryEvent|ImportedEvent|PokerusEvent|ExpShareEvent
 *   |(LevelEvent & { fromLevel: number })
 *   |(EvolveEvent & { fromName: string, toName: string, sprite: string|null, level: number })} HistoryRecord */
 ...
 * @property {HistoryRecord[]} history
 */
 ...
 * `overrides` is only `Partial` here (unlike the always-fully-populated `Party.overrides`)
 * because a payload from another device/app version can predate a since-added override key
 * — applyImport/`_migrate` both defensively fill in `defaultOverrides()` around it.
 * @typedef {Omit<Party, 'pokemon'|'overrides'> & { overrides: Partial<PartyOverrides>, pokemon: PersistedEntry[] }} ExportedParty */
...
/** @param {number|string} level @param {number} fallback @returns {number} */
 ...
 * @typedef {(acc: FoldAcc, ev: RosterEvent) => HistoryRecord} EventHandler
 */
...
export class Store extends EventTarget {
   ...
   * @template {RosterEvent} T
   * @param {RosterEntry} entry @param {T} event @returns {T} */
  _append(entry, event) {
  ...
  /** @param {string} uid @param {number|string} level */
  setLevel(uid, level) {
    const entry = this._find(uid);
    if (!entry) return;
    const parsed = Math.round(Number(level));
   ...
   * @returns {{ base: EvMap, afterItem: EvMap, afterPokerus: EvMap, applied: EvMap }|null}
   */
   ...
   */
  /** @param {string} uid @param {import('./pokeapi-client.js').DomainPokemon} opponent @returns {BattleEvent|null} */
   ...
   */
  /** @param {string} uid @param {string} vitaminId @returns {VitaminEvent|null} */
  useVitamin(uid, vitaminId) {
    ...
    const blockedByCeiling = statExp && entry.evs[vitamin.stat] >= STAT_EXP_VITAMIN_CEILING;
    ...
    return this._append(
      entry,
      makeEvent('vitamin', { vitaminId: vitamin.id, stat: vitamin.stat, linkedStat, applied, blockedByCutoff, blockedByCeiling })
    );
  }
   ...
   */
  /** @param {string} uid @param {string} featherId @returns {FeatherEvent|null} */
   ...
   */
  /** @param {string} uid @param {string} berryId @returns {BerryEvent|null} */
