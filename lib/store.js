...
export class Store extends EventTarget {
  ...
  catchPokemon(mon, level = DEFAULT_LEVEL, natureId = null) {
    ...
    const entry = {
      ...
      level: Number.isNaN(parsedLevel) ? DEFAULT_LEVEL : Math.min(MAX_LEVEL, Math.max(MIN_LEVEL, parsedLevel)),
  ...
  /**
   * Deletes one history record (battle or vitamin) from the caught
   * Pokémon `uid` — for a mislogged entry — and reverts the EVs it
   * applied, so the roster stays consistent with what's actually
