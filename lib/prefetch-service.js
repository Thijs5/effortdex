...
// docs/adr/0012 for the manual per-game trigger (pages/sprite-cache.js)
// added alongside it. sw.js caches a sprite the moment it's ever
// requested, but a species nobody has looked up yet — caught, searched,
...
export class PrefetchService extends EventTarget {
  ...
  /**
   * Manual trigger (pages/sprite-cache.js, ADR 0012): prefetches every
   ...
   * render cache-status counts (pages/sprite-cache.js) without
   ...
   * cache controls in pages/sprite-cache.js, "Generation IV" meaning
  ...
  /** Whether the queue is currently sitting out a backoff cooldown after repeated failures — exposed for UI (pages/sprite-cache.js) to explain a stalled "Caching…" state rather than leaving it looking frozen.
   * @returns {boolean} */
  get isBackingOff() {
